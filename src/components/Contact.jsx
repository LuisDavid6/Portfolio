...
const Contact = ({ theme }) => {
  const shadow = theme !== 'dark' ? '0px 5px 25px rgba(0, 0, 0, 0.3)' : '0px 5px 25px rgba(0, 0, 0, 35)'

  return (
    <div id='contact' className='flex flex-col items-center mt-16'>
      <h2 className='text-4xl p-1 mb-14 font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-violet-500 dark:from-great-blue-500 dark:via-violet-600 dark:to-emerald-700'>
        Contacto
      </h2>

      <div className='flex flex-wrap w-full justify-center gap-16 '>
        <motion.div animate={{ boxShadow: shadow }} className=' p-4 mt-24 dark:text-white h-full rounded-2xl'>
          ...
          <div className='flex gap-3 mb-4'>
            <RiGithubFill color={theme === 'dark' ? 'white' : 'black'} size={40} />
